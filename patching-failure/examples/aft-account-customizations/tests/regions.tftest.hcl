...
mock_provider "aws" {
  ...
  mock_data "aws_iam_account_alias" { defaults = { account_alias = "pilot" } }
  mock_resource "aws_lambda_function" { defaults = { arn = "arn:aws:lambda:us-west-2:222233334444:function:pilot-patch-outcome-us-west-2-writer" } }
  mock_resource "aws_cloudwatch_log_group" { defaults = { arn = "arn:aws:logs:us-west-2:222233334444:log-group:/aws/lambda/pilot-patch-outcome-us-west-2-writer" } }
  mock_resource "aws_sqs_queue" { defaults = { arn = "arn:aws:sqs:us-west-2:222233334444:mock-queue" } }
  mock_resource "aws_cloudwatch_event_rule" { defaults = { arn = "arn:aws:events:us-west-2:222233334444:rule/mock-rule" } }
}
variables {
  member_account_id   = "222233334444"
  organization_id     = "o-example1234"
  archive_bucket_name = "central-patching-logs-111122223333"
}

run "two_regions_one_identity" {
  command = apply
  assert {
