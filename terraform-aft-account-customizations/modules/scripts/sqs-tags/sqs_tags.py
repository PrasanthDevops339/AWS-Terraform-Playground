...
def evaluate_compliance(configuration_item, rule_parameters):
    ...
    if missing_tag_keys:
        return {
            ...
            "annotation": f"Required SQS tag keys are missing: {', '.join(missing_tag_keys)}"
        }
    ...
    if non_compliant_tags:
        return {
            "compliance_type": "NON_COMPLIANT",
            "annotation": f"SQS tag values do not match accepted values: {', '.join(non_compliant_tags)}"
        }

    return {
        "compliance_type": "COMPLIANT",
        "annotation": "All required SQS tags have values matching accepted values."
    }
