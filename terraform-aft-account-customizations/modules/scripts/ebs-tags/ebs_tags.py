...
def evaluate_compliance(configuration_item, rule_parameters):
    ...
    if missing_tag_keys:
        return {
            ...
            "annotation": f"Required EBS tag keys are missing: {', '.join(missing_tag_keys)}"
        }
    ...
    if non_compliant_tags:
        return {
            "compliance_type": "NON_COMPLIANT",
            "annotation": f"EBS tag values do not match accepted values: {', '.join(non_compliant_tags)}"
        }

    return {
        "compliance_type": "COMPLIANT",
        "annotation": "All required EBS tags have values matching accepted values."
    }
