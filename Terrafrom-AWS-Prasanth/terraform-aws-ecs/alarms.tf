...
locals {
  ...
  # RunningTaskCount is only published to the ECS/ContainerInsights namespace.
  # Creating that alarm with Container Insights disabled yields an alarm stuck
  # permanently in ALARM on missing data, so it is gated on the cluster setting.
  container_insights_enabled = anytrue([
    for s in flatten([var.cluster_settings]) :
    s.name == "containerInsights" && contains(["enabled", "enhanced"], s.value)
  ])

  task_count_alarm_services = {
    for k, v in local.alarm_services : k => v
    if local.container_insights_enabled
  }
}
...
resource "aws_cloudwatch_metric_alarm" "task_count_low" {
  for_each = local.task_count_alarm_services
