...
variable "api_ecs_alb_service_role_arn" {
  description = <<-EOT
    Optional. Infrastructure IAM role ARN that lets ECS shift ALB traffic
    during CANARY, LINEAR and BLUE_GREEN deployments.

    Leave null and the module creates one, with
