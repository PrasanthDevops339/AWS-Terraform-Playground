...
locals {
  # Autoscaling works with every deployment controller. DAEMON services are the
  # exception: ECS already runs exactly one task per container instance, so
  # there is no desired count to scale.
  autoscaling_services = {
    for k, v in var.container_config : k => try(v.autoscaling, null)
    if try(v.autoscaling, null) != null && !local.svc_resolved[k].is_daemon
  }
...
resource "aws_appautoscaling_target" "main" {
  ...
  resource_id        = "service/${local.cluster_name_full}/${local.service_names[each.key]}"
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace  = "ecs"

  tags = merge(var.tags, { "Name" = "${local.account_alias}-${each.key}" })

  # The service must exist and be registered before it can be a scaling target.
  depends_on = [
    aws_ecs_service.main,
    aws_ecs_service.main_unmanaged_td,
