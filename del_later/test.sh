...
EXPECTED_ROLE_NAME="AWSSystemsManagerDefaultEC2InstanceManagementRole"
...
if CALLER_JSON=$(aws sts get-caller-identity --output json 2>&1); then
  ...
  if [[ "$CALLER_ARN" == *"$EXPECTED_ROLE_NAME"* ]]; then
    ok "Caller role matches expected: $EXPECTED_ROLE_NAME"
  else
    bad "Caller role does NOT match expected role name '$EXPECTED_ROLE_NAME'"
    info "Bucket policy ArnEquals condition will silently fail to match this identity."
  fi
else
